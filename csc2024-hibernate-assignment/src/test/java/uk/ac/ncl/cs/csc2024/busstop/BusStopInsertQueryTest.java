...
/**
 * Class containing tests of the BusStop insert queries.
 *
 ...
 * @author hugofirth
 */
public class BusStopInsertQueryTest {
    ...
    public void setUp() throws IOException, URISyntaxException {
        URL csvUrl = getClass().getResource("/busstops.csv");
    ...
    public void testInsertedCorrectRows() {
        try {
            ...
            for (Map<String, String> row : rows) {
                BusStopQueries.insert(row, session);
            }
            session.getTransaction().commit();
            session.beginTransaction();
            Query query = session.createQuery("from BusStop b");
            assertTrue(BusStopPredicates.INSERT_CORRECT_ROWS.isSatisfied(query));
            session.getTransaction().commit();
            ...
            session.getTransaction().rollback();
            fail("An exception was thrown because some of the BusStops rows could not be inserted into the " +
